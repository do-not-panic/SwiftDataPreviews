...
//  Books
//
//  Created by René Pfammatter on 06.11.2024.
//
...
struct BookListView: View {
    ...
    init(sortOrder: SortOrder, filterType: FilterType, filter: String) {
        ...
        switch sortOrder {
            case .book: [SortDescriptor(\Book.name)]
        case .genre: [SortDescriptor(\Book.genre.name)]
        }
        ...
        if filter.isEmpty {
            ...
            case .author:
                let foundAuthorsCount = #Expression<[Author],Int> { authors in
                    authors.filter {
                        $0.firstName.localizedStandardContains(filter) ||
                        $0.lastName.localizedStandardContains(filter)
                    }.count
                }
                predicate = #Predicate<Book> { book in
                    book.authors.count > 0 && foundAuthorsCount.evaluate(book.authors) > 0
                }
            }
        }
...
#Preview(traits: .mockData) {
    ...
    let filter = ""
    BookListView(sortOrder: sortOrder, filterType: filterType, filter: filter)
}
